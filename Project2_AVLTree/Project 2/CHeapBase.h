...
public:
	...
	*	@param	item	MusicType형의 새로운 데이터.
	*	@return	잘 작동하면 1을 반환
	*/
	...
	*	@param	item	MusicType형의 삭제 할 데이터.
	*	@return	잘 작동하면 1을 반환
	*/
	virtual int Delete(T item);

	/**
	*	@brief	Heap의 데이터를 변경하는 가상함수.
	*	@pre	Heap가 초기화되어야한다.
	*	@post	Heap의 데이터를 변경한다.
	*	@param	item	MusicType형의 변경 할 데이터.
	*	@return	잘 작동하면 1을 반환
	*/
	virtual void ReplaceItem(T& item, bool &found);
	...
	*	@param	item	MusicType형의 검색 할 데이터.
...
// Heap의 데이터를 삭제하는 가상함수.
template <typename T>
void CHeapBase<T>::ReplaceItem(T& item, bool &found)
{
	Replace(item, found, 0); // 데이터 검색
	ReheapUp(0, m_iLastNode - 1); // 데이터 정렬
}
